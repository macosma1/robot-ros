...
class Robot(object):
    ...
    def scan_data(self):
        ...
        if self.__step_cache == self.step and not self.force_update:
            ...
            data_front = None
            data_back = None
            # while (data_front is None and data_back is None):
            while (data_front is None):
                try:
                    data_front = rospy.wait_for_message('/camera', LaserScan, timeout=5)
                    data_back = rospy.wait_for_message('/back_camera', LaserScan, timeout=5)
                    #data_front1 = rospy.Subscriber('/camera/scan', LaserScan, timeout=5)
                    # print(data_front)
                    #print(data_back)
                    #print(data_front1)
                except:
            ...
            for i in range(len(scan.ranges)):               #back camera
                if scan2.ranges[i] == float("Inf"):
                    scan_data.append(3.5)
                elif np.isnan(scan2.ranges[i]):
                    scan_data.append(0)
                else:
                    scan_data.append(scan2.ranges[i])
